...
## Конфігурація
Для налаштування параметрів проекту "StudentFlow" можна використовувати файл .env. Цей файл є текстовим файлом, який зберігає конфігураційні змінні середовища.

Щоб налаштувати проект, створіть файл з назвою .env у кореневій директорії проекту та додайте в нього рядки наступного формату:
...
1. Переконайтесь, що у вас встановлено Python версії 3.10+. Якщо ні, завантажте та встановіть його з офіційного сайту Python (https://www.python.org/).
...
4. Встановіть необхідні пакети з файлу `requirements.txt`, використовуючи команду:
```shell
pip install -r requirements.txt
```
Ця команда встановить всі залежності, необхідні для проєкту Django, зазначені в файлі requirements.txt.
...
1. Відкрийте командний рядок (термінал) на вашому комп'ютері.
2. Перейдіть до папки проєкту, використовуючи команду `cd /шлях/до/папки/проєкту`.
3. Переконайтеся, що у вас встановлено Redis на вашому комп'ютері перед запуском скрипту. Якщо Redis не встановлено, завантажте та встановіть його з офіційного сайту Redis (https://redis.io/).
...
### Крок 4: Запуск проєкту Django

1. У командному рядку перейдіть до папки, де міститься файл manage.py.
2. Запустіть локальний сервер Django, використовуючи команду:
```shell
python manage.py runserver
```
Ця команда запустить сервер на локальному комп'ютері за адресою http://127.0.0.1:8000/.
3. Запустити проєкт глобально можна використовуючи команду:
```shell
